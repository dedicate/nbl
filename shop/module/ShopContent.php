...
function ShopContent(){
	...
	//获取同类产品（包括此产品）
	$stl=" iffb='1' and catid!='0' and id='$id' ";
	$nums=0;
	$msql->query("select * from {P}_shop_con where $stl");
	if($msql->next_record()){
		$sid=$msql->f('id');
		$ssrc=$msql->f('src');
		$ssrc=ROOTPATH.$ssrc;
	$pics="<div class='box'><div class='fang'><div class='picFit'><img src='".$ssrc."' width='52' height='52' onClick='changePic(\"".$sid."\",\"shopview\",\"first\"); ' /></div></div><input type='hidden' id='dp_".$nums."' value='".$sid."' /></div>";					
	}
	
	$stl=" gid='$id' ";
	$msql->query("select * from {P}_shop_pages where $stl order by id asc");
	while($msql->next_record()){
		$sid=$msql->f('id');
		$ssrc=$msql->f('src');
		$ssrc=ROOTPATH.$ssrc;
		$nums++;
			$pics.="<div class='box'><div class='fang'><div class='picFit'><img src='".$ssrc."' width='52' height='52' onClick='changePic(\"".$sid."\",\"shopview\",\"\");  ' /></div></div><input type='hidden' id='dp_".$nums."' value='".$sid."' /></div>";
